...
"""
I94PORT values from I94_SAS_Labels_Descriptions.SAS
"""
CitiesAndStates = {
       'ALC':	'ALCAN, AK             ',
       'ANC':	'ANCHORAGE, AK         ',
...
def correct_to_integer(df, columns):
    """
    Casts given list of columns of a dataframe to integer
    """
...
def udfExtractLatLongFromCoords(coord):
    """
    Extracts latitude/longitude from coordinates column.
    """
    if coord == None:
        return None
...
def udfExtractStateFromRegion(iso_region):
    """
    Extracts the (US) state from a given iso region code.
    """
    if iso_region == None:
        return None
    
    values = iso_region.split('-')
    if len(values) < 2:
        return None
    return values[1].lower()

@udf(returnType=StringType()) 
def udfI94Mode(mode_as_int):
    """
    Converts to I94MODE values (see I94_SAS_Labels_Descriptions.SAS)
    """
    if mode_as_int == None:
        return 'not reported'
    ...
    return values[mode_as_int].lower()

@udf(returnType=StringType()) 
def udfI94VISA(visa):
    """
    Converts to I94VISA values (see I94_SAS_Labels_Descriptions.SAS)
    """
    if visa == None:
        return 'none'
    ...
    return values[visa].lower()

@udf(returnType=StringType())
def udfDestCity(code):
    """
    Extracts the city from I94PORT 
    """
    if code == None:
        return None
    
    if code in CitiesAndStates:
        cityOrState = CitiesAndStates[code].lower().split(',')
        if len(cityOrState) < 2:
            return None
        
        return cityOrState[0]
    return None

@udf(returnType=StringType())
def udfDestState(code):
    """
    Extracts the state from I94PORT 
    """
    if code == None:
        return None
    ...
    if code in CitiesAndStates:
        ...
        return cityOrState[1]
    
    return None

@udf(returnType=StringType()) 
def udfCityOrResident(code):
    """
    Extracts the country of origin from I94RES 
    """
    if code == None:
        return None
